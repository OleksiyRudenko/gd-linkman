 ...
 * inability to access GOS documents when vis-a-vis has got no Google account
 ...
 * Convert GOS files into Microsoft Office or other 
   types on the fly.
...
| GOS file type       | MSO         | OpenDoc | html  | rtf / epub | pdf   | txt | csv / tsv | jpg / png / svg |
|---------------------|:-----------:|:-------:|:-----:|:----------:|:-----:|:---:|:---------:|:---------------:|
| Docs                | docx        | odt     | `(1)` | +          | +     | +   | -         | -               |
 ...
 * `open` - equivalent to shareable link available from
   Google Drive context menu. GSO documents are get opened with
   native google applications. Other files are get opened with
   Drive preview or associated application if any.
 * `download` - direct file download link. GSO documents can
   be exported in either of
   [supported formats](#gos-export-support), so user is presented
 ...
 * `open` : preview|GOS open
 * `DL`   : download for non-GOS files

For GOS documents exporting urls are stored per export format.
MSO and pdf export urls are generated by default. Other types
require manual activation.

goo.gl stats available online.
