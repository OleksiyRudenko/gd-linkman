...
var module = angular.module('linkman.drive', ['linkman.gapi']);
...
module.service('drive', ['$q', '$cacheFactory', 'googleApi', 'applicationId', function($q, $cacheFactory, googleApi, applicationId) {
  ...
  /**
   * Load a file from Drive. Fetches the metadata only.
   *
   * @param {String} fileID ID of the file to load
   * @return {Promise} promise that resolves to an object containing the file metadata
   */
  this.loadFileMetaData = function(fileId) {
    ...
    return googleApi.then(function(gapi) {
      ...
      return $q.all($q.when(metadataRequest));
